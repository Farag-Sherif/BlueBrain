...
const Header = () => {
  ...
  const isAiPage = location.pathname.toLowerCase() === "/ai-booth-designer";
      ...
      >
        <nav aria-label="Global" className="flex items-center justify-between container mx-auto px-4">
          <Link to="/" className="-m-1.5 p-1.5 flex-shrink-0">
            <img src={logo} alt="Blue Brain Logo" className={`h-10 sm:h-12 w-auto object-contain hover:scale-105 transition-transform duration-300 ${isScrolled || isAiPage ? "brightness-100" : "brightness-100"}`} />
          </Link>
